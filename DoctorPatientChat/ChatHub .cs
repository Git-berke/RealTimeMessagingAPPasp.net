...
namespace DoctorPatientChat
{
    public class ChatHub : Hub
    {
        ...
        {
            string receiverUsername = Context.User.Identity.Name;

            if (string.IsNullOrEmpty(senderUsername) || string.IsNullOrEmpty(receiverUsername))
                return;

            var senderData = UserManager.GetUserByUsername(senderUsername);
            var receiverData = UserManager.GetUserByUsername(receiverUsername);

            if (senderData != null && receiverData != null)
            {
                int senderId = Convert.ToInt32(senderData["UserID"]);
                int receiverId = Convert.ToInt32(receiverData["UserID"]);

                // Mesajları okundu olarak işaretle
                MessageManager.MarkMessagesAsRead(receiverId, senderId);

                // Gönderene mesajların okunduğu bilgisini gönder
                Clients.User(senderUsername).messagesRead(receiverUsername);
            }
        }
    }
}